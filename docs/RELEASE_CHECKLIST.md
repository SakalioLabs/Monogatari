...
- [ ] `cargo test --locked -p monogatari-mcp` passes, including a real stdio child-process handshake, eleven schema-backed tools, scrubbed reads, structured core/delivery validation, bounded Quality execution with exact source SHA and actionable pass/failure evidence, deterministic plans, default write refusal, reviewed transaction/package fingerprint enforcement, fixed-root package inspection/output with traversal, damaged-archive, and implicit-replacement rejection, one-writer exclusion, successful core-runtime application, and rollback after document or runtime-reference rejection
