...
- [ ] `cargo test --locked -p monogatari-mcp` passes, including a real stdio child-process handshake, fourteen schema-backed tools, scrubbed reads, structured core/delivery validation, source-bound provider-free Scene Roleplay and Workflow previews, bounded Quality execution, deterministic plans, default write refusal, reviewed transaction/package fingerprints, fixed-root package inspection/private runtime validation/output, one-writer exclusion, successful core-runtime application, and rollback
...
- [ ] MCP inspection claims document acceptance; candidate application claims only `core_runtime`; `preview_scene_roleplay` reports provider-free state-machine execution bound to one roleplay source SHA; `preview_workflow` reports provider-free graph execution; `run_quality_suite` reports deterministic roleplay/Workflow evidence; live generation, package, installation, and rendered-experience gates retain their documented boundaries
