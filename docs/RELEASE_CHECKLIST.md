...
- [ ] Release artifact manifest generated from a clean tracked git worktree with SHA-256 checksums, checked-in Quality Suite source evidence plus aggregate suite-set fingerprints, checked-in release channel policy metadata, source-state evidence, installer expectations, and signing evidence
