...
- [ ] Rust `monogatari-game-save/v2` snapshots restore scene history, dialogue cursor/local state, typed Rhai variables, character emotion/relationships/full memory, chat history, evaluations, safety traces, and triggered event IDs while schema-less v1 saves remain readable
