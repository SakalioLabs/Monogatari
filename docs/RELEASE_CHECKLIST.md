...
- [ ] Release artifact manifest generated from a clean tracked git worktree with SHA-256 checksums, checked-in Quality Suite and workflow source evidence plus aggregate source-set fingerprints, checked-in release channel policy metadata, source-state evidence, installer expectations, and signing evidence
