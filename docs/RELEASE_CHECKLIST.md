...
- [ ] `cargo test --locked -p llm-authoring` passes independently of Tauri, including structured bounded Dialogue and Knowledge authoring evidence plus bounded Knowledge document loading/runtime construction
...
- [ ] `cargo test --locked -p monogatari-mcp` passes, including a real stdio child-process handshake, thirteen schema-backed tools, scrubbed reads, structured core/delivery validation, provider-free Workflow preview and bounded Quality execution with exact source SHAs and actionable trace/pass/failure evidence, deterministic plans, default write refusal, reviewed transaction/package fingerprint enforcement, fixed-root package inspection/private runtime validation/output with temporary cleanup plus traversal, damaged-archive, invalid-runtime, and implicit-replacement rejection, one-writer exclusion, successful core-runtime application, and rollback after document, Dialogue, Knowledge, or cross-catalog runtime rejection
...
- [ ] MCP inspection claims document acceptance; candidate application claims only `core_runtime` catalog and reference acceptance; `preview_workflow` reports provider-free graph execution bound to one project source SHA; `run_quality_suite` separately reports deterministic Quality execution bound to one project source SHA; package preview/export reports manifest and streamed archive evidence bound to one package fingerprint; fixed-root package inspection reports archive integrity without extraction; private package validation reports ephemeral extraction plus shared core-runtime/delivery evidence without installation; persistent installation and rendered-experience gates retain their documented boundaries
...
- [ ] Checked-in Knowledge `related_entries` and legacy `relatedEntries` resolve to existing non-self entries, and deterministic search/list ordering is identical after desktop, core-runtime, and package loading
...
- [ ] Shared headless package inspection/extraction rejects traversal, non-portable or case-colliding paths, encrypted/symbolic/special ZIP entries, undeclared files, malformed JSON, runtime secrets, checksum/fingerprint mismatches, non-empty extraction roots, and declared file/count/expanded-size bombs
