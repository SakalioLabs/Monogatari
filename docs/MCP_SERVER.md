...
Scene Roleplay changes are accepted only when the shared game schema and authoring validator approve score/evidence IDs and bounds, node reachability and local targets, turn/inference limits, intrusion responses, forbidden/grounding output guards, rotating recoveries, fallback signal references/deltas, and every scene, character, Knowledge, and ending reference. Normal NPC responses remain generated prose. Live runtime turns commit only when NPC generation and independent structured evaluation both succeed; inference, guard, or parsing failure leaves story state unchanged. Authored recovery and fallback signals are reserved for provider-free Quality/MCP simulation, attacks always receive zero score/evidence, and only the shared deterministic state machine can select a transition or ending.
