...
|---|---|---|---|
| Agent and test orchestration | `automation-contracts` | Node unit contracts | Matrix schema, ownership, selection, CLI parsing, platform command adaptation, fail-closed release-channel/manifest evidence, and multi-provider CSP/hosting policy validation |
...
3. Continue decomposing `scripts/verify-release.mjs`; frontend, AI, and path source-invariant checks plus release-channel and Web hosting policy validation now live in importable modules, while content, packaging, and live browser gates remain in the entry point.
