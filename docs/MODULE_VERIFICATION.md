...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, and Tauri packaging evidence now live in importable modules. The project-content gate remains in `verify-release.mjs`; package/mobile, installation, and complete command-registration policies are tested Tauri submodules, while project/runtime integration and runtime-safety rule groups still need extraction from the parent collector.
