...
2. Continue moving schema-specific catalog validation, cross-reference discovery, and packaging behind `llm-authoring`; project settings, portable paths, package manifest semantics, JSON document inspection, and transactions are now shared without transport duplication, while ZIP I/O remains Tauri-only.
