...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, and Tauri packaging evidence now live in importable modules. The project-content gate remains in `verify-release.mjs`; package/mobile policy is now a tested Tauri submodule, while installation, command-registration, and runtime-safety rule groups still need extraction from the parent collector.
