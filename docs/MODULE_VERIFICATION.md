...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, Story Event/Ending, Dialogue, Workflow, Renderer Asset, Knowledge Reference, and Tauri packaging evidence now live in importable modules. The Tauri collector decomposition is complete under a pure parent orchestrator. Quality Suite source policy and repository-wide JSON parsing remain project-content rule domains owned by `verify-release.mjs`.
