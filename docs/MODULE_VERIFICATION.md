...
2. Continue moving schema-specific catalog validation, cross-reference discovery, and package transport behind `llm-authoring`; project settings, portable paths, package inventory/manifest generation and validation, JSON document inspection, and transactions are now shared without transport duplication, while ZIP I/O and the user-facing package command remain Tauri-only and MCP does not yet emit archives.
