...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, and Tauri packaging evidence now live in importable modules. The project-content gate remains in `verify-release.mjs`; package/mobile, installation, complete command registration, conversation safety, headless Quality/Workflow, and build/toolchain policies are tested Tauri submodules, while project/package runtime integration and Story Event authoring integration still need extraction from the parent collector.
