...
| Standard MCP adapter | `rust-mcp` | Unit, protocol, and real stdio child-process tests | Fixed project root, eleven schema-backed tools, project-external cross-process leases, core/delivery validation, bounded Quality execution/evidence, reviewed transaction and package fingerprints, fixed external package inspection/output, damaged-archive rejection, write exclusion, candidate validation, and rollback |
