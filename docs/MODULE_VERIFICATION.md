...
|---|---|---|---|
| Agent and test orchestration | `automation-contracts` | Node unit contracts | Matrix schema, ownership, selection, CLI parsing, platform command adaptation, structural frontend route/sidebar coverage, fail-closed release-channel/manifest evidence, multi-provider CSP/hosting policy validation, static Web distribution evidence, Web preview route/content response evidence, independently injected Tauri package/mobile, installation, exact 111-command registration, dialog capabilities, 79-requirement conversation safety, 60-requirement headless Quality/Workflow, eight-requirement build/toolchain, 53-requirement project runtime, and 105-requirement project package policy |
...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, and Tauri packaging evidence now live in importable modules. The project-content gate remains in `verify-release.mjs`; package/mobile, installation, complete command registration, conversation safety, headless Quality/Workflow, build/toolchain, project runtime, and project package policies are tested Tauri submodules. Story Event authoring/runtime integration is the final rule domain still owned by the parent Tauri collector.
