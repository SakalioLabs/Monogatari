...
|---|---|---|---|
| Agent and test orchestration | `automation-contracts` | Node unit contracts | Matrix schema, ownership, selection, CLI parsing, platform command adaptation, structural frontend route/sidebar coverage, fail-closed release-channel/manifest evidence, multi-provider CSP/hosting policy validation, static Web distribution evidence, Web preview route/content response evidence, cross-root Story Event/Ending fingerprint and Dialogue graph/reference evidence, independently injected Tauri package/mobile, installation, exact 111-command registration, dialog capabilities, 79-requirement conversation safety, 60-requirement headless Quality/Workflow, eight-requirement build/toolchain, 53-requirement project runtime, 105-requirement project package, and 93-requirement Story Content policies |
...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, Story Event/Ending, Dialogue, and Tauri packaging evidence now live in importable modules. The Tauri collector decomposition is complete under a pure parent orchestrator. Workflow shape, renderer assets, Knowledge references, Quality Suite source policy, and repository-wide JSON parsing remain project-content rule domains owned by `verify-release.mjs`.
