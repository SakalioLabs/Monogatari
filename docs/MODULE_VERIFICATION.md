...
| Tauri application | `rust-tauri`, `rust-tauri-check` | Command/project tests and compile check | Desktop commands adapt state into shared authoring domains; Scene, Dialogue, and Ending saves prove portable case aliases cannot replace existing documents; provider calls, persistent runtime effects, remaining catalogs, and package ZIP/filesystem orchestration stay in the command crate |
...
2. Continue moving schema-specific catalog validation, cross-reference discovery, and package transport behind `llm-authoring`; project settings, portable paths, package inventory/manifest generation, validation, and ZIP output, JSON document inspection, and transactions are now shared without transport duplication, while archive inspection/import remain Tauri-only and MCP does not yet expose the shared package writer.
