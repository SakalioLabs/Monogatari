...
3. Continue decomposing release verification; frontend, AI, path, route, release-channel, Web hosting/distribution/preview, and Tauri packaging evidence now live in importable modules. The project-content gate remains in `verify-release.mjs`; package/mobile and installation policies are now tested Tauri submodules, while command-registration and runtime-safety rule groups still need extraction from the parent collector.
