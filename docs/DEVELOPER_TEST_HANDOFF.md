...
1. Start `monogatari-mcp` with one fixed project root; omit `--allow-write` for inspection-only clients.
...
6. Repeat both validators and run package, Quality, and rendered-experience gates appropriate to the deliverable.

`document` acceptance proves bounded JSON safety. `core_runtime` acceptance additionally proves real catalog loading and cross-reference validation. Delivery acceptance proves declared asset readiness. None of these levels alone claims rendered visual quality or provider-backed generation.
