...
This validates JSON assets, checked-in workflow files, all quality suite files, workflow branch coverage snapshots, locale coverage, frontend UI text artifacts, frontend source invariants, legacy C# AI prompt/API invariants, mobile shell readiness, Tauri mobile deployment preflight, Rust AI/game/Tauri checks and tests, frontend audit, the Web/PWA build, generated dist assets, responsive shell layout signals, release artifact manifest checks, and legacy C# tests.
