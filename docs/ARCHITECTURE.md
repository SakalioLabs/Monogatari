...
Project-package commands use a thin Tauri adapter under `project_archive/commands.rs`. That adapter owns application-state reads, blocking ZIP task dispatch, staged runtime reload validation, and the final non-overwriting directory commit. Pure portable-path limits, reserved-name checks, case-folded comparison keys, and parent expansion live in `project_archive/path_validation.rs`; the parent archive module owns package models, manifest validation, streaming ZIP I/O, checksums, and atomic package replacement. This keeps desktop command registration and reusable path policy separate from the filesystem mechanics exercised by archive round-trip and tamper tests.
