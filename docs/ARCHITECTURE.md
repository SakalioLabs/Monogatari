...
Project-package commands use a thin Tauri adapter under `project_archive/commands.rs`. That adapter owns application-state reads, blocking ZIP task dispatch, staged runtime reload validation, and the final non-overwriting directory commit; the parent archive module owns package models, manifest and portable-path validation, streaming ZIP I/O, checksums, and atomic package replacement. This keeps desktop command registration separate from the filesystem mechanics exercised by archive round-trip and tamper tests.
