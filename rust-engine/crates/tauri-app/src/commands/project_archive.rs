...
use super::project::{
    build_project_config_state, build_project_export_manifest, project_export_settings_bytes,
    resolve_project_root, sanitize_export_config, scrub_runtime_secret_config,
};
...
pub async fn import_project_archive(
    ...
    if let Err(error) = super::engine::load_project_content(&staging_root).await {
        remove_import_staging(&staging_root);
        return Err(format!("Imported runtime content is invalid: {error}"));
    }
    if let Err(error) = super::scenes::build_scene_asset_catalog(&staging_root) {
        remove_import_staging(&staging_root);
        return Err(format!("Imported scene content is invalid: {error}"));
    }
    if let Err(error) = super::endings::load_story_endings(&staging_root) {
        remove_import_staging(&staging_root);
