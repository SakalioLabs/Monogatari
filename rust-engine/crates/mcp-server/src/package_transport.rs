//! Fixed-root MCP transport for project-package preview and export.

use std::path::{Path, PathBuf};

use llm_authoring::project_package::{
    build_project_export_manifest, validate_manifest, validate_portable_path,
    write_project_package, ProjectExportRuntimeSnapshot, ProjectPackageTargetPolicy,
};

use crate::protocol::{
    ExportProjectPackageOutput, ExportProjectPackageRequest, McpToolError,
    PreviewProjectPackageOutput, MCP_PACKAGE_EXPORT_SCHEMA_V1, MCP_PACKAGE_PREVIEW_SCHEMA_V1,
};
...
#[derive(Debug, Clone)]
pub(crate) struct PackageOutputBoundary {
    root: PathBuf,
}

impl PackageOutputBoundary {
    pub(crate) fn new(project_root: &Path, output_root: PathBuf) -> Result<Self, String> {
        let root = canonical_regular_directory(&output_root, "MCP package output directory")?;
        if root.starts_with(project_root) {
            return Err(
                "MCP package output directory must stay outside the authored project root."
                    .to_string(),
            );
        }
        Ok(Self { root })
    ...
    fn destination(&self, file_name: &str) -> Result<PathBuf, McpToolError> {
        validate_package_file_name(file_name)?;
        let current_root = canonical_regular_directory(&self.root, "MCP package output directory")
            .map_err(|message| McpToolError::package(message, None))?;
        if current_root != self.root {
            return Err(McpToolError::package(
                "MCP package output directory changed after server startup.",
                None,
            ));
        }
...
pub(crate) async fn export_project_package(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let destination = output.destination(&request.file_name)?;
...
fn validate_package_file_name(file_name: &str) -> Result<(), McpToolError> {
    ...
    if path.file_name().and_then(|name| name.to_str()) != Some(file_name) {
        return Err(McpToolError::package(
            "Package output accepts one file name, not a filesystem path.",
            None,
        ));
    }
...
mod tests {
    ...
    #[test]
    fn output_boundary_stays_external_and_accepts_only_portable_package_names() {
        ...
        assert!(PackageOutputBoundary::new(&project, project.join("packages")).is_err());
        let boundary = PackageOutputBoundary::new(&project, output.clone()).unwrap();
        assert!(boundary.destination("story.monogatari").is_ok());
        assert!(boundary.destination("../story.monogatari").is_err());
        assert!(boundary.destination("story.zip").is_err());
        assert!(boundary.destination("CON.monogatari").is_err());
