...
"#;
    ...
    assert!(matches!(&commands[0], llm_game::script::ScriptCommand::ChangeBg { .. }));
    
    // Verify last command is End
    assert!(matches!(commands.last().unwrap(), llm_game::script::ScriptCommand::End));
    
    // Verify dialogue commands
    let say_commands: Vec<_> = commands.iter()
