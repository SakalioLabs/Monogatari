  ...
  <div class="audio-page">
    ...
    <section class="audio-grid">
      <div class="audio-section">
        ...
        <div v-for="track in bgmTracks" :key="track.id" class="track-card" :class="{ playing: isTrackPlaying(track), warning: trackErrors[track.id] }">
          <div class="track-info">
            <span class="track-icon">{{ trackTypeLabel(track.type) }}</span>
          ...
          <div class="track-controls">
            <button class="ctrl-btn" :title="musicButtonLabel(track)" @click="togglePlay(track)">{{ isTrackPlaying(track) ? '||' : '>' }}</button>
      ...
      <div class="audio-section">
        ...
        <div v-for="track in sfxTracks" :key="track.id" class="track-card" :class="{ playing: isSfxPreviewing(track.id), warning: trackErrors[track.id] }">
          ...
          <div class="track-controls">
            <button class="ctrl-btn" title="Preview" @click="playSfx(track)">>|</button>
            <input type="range" :value="track.volume" min="0" max="100" @input="setVolume(track, $event)" class="vol-slider" />
            <span class="vol-label">{{ track.volume }}%</span>
          </div>
        </div>
    ...
    <section class="mixer-panel">
      ...
      <div class="mixer-grid">
        <label class="mixer-channel"><span>Master</span><input type="range" v-model.number="masterVolume" min="0" max="100" /><b>{{ masterVolume }}%</b></label>
        <label class="mixer-channel"><span>BGM</span><input type="range" v-model.number="bgmVolume" min="0" max="100" /><b>{{ bgmVolume }}%</b></label>
        <label class="mixer-channel"><span>Ambient</span><input type="range" v-model.number="ambientVolume" min="0" max="100" /><b>{{ ambientVolume }}%</b></label>
        <label class="mixer-channel"><span>SFX</span><input type="range" v-model.number="sfxVolume" min="0" max="100" /><b>{{ sfxVolume }}%</b></label>
        <label class="mixer-channel"><span>Voice</span><input type="range" v-model.number="voiceVolume" min="0" max="100" /><b>{{ voiceVolume }}%</b></label>
      </div>
      <div class="transport-row">
        <span>{{ playingMusicLabel }}</span>
        <button class="btn btn-secondary btn-sm" :disabled="!playingMusicId" @click="stopMusic">Stop</button>
      </div>
...
const playingMusicLabel = computed(() => {
  ...
  return track ? `Playing ${track.name}` : 'No music playing'
})
...
function trackTypeLabel(type: AudioTrackType) {
  if (type === 'sfx') return 'S'
  if (type === 'ambient') return 'A'
  return 'B'
}
...
function musicButtonLabel(track: AudioTrack) {
  return isTrackPlaying(track) ? 'Pause' : 'Play'
}
...
function createAudioElement(track: AudioTrack): HTMLAudioElement | null {
  ...
  if (!resolvedUrl) {
    setTrackError(track.id, 'Missing audio path')
    return null
  }
  ...
  audio.onerror = () => {
    setTrackError(track.id, 'Audio file unavailable')
    lastAudioError.value = `${track.name}: audio file unavailable`
  }
  return audio
}
...
async function playMusic(track: AudioTrack) {
  ...
  try {
    ...
    stopMusic()
    setTrackError(track.id, 'Playback blocked')
    lastAudioError.value = `${track.name}: playback blocked or unsupported`
...
async function playSfx(track: AudioTrack) {
  ...
  try {
    ...
    setTrackError(track.id, 'Preview blocked')
    lastAudioError.value = `${track.name}: preview blocked or unsupported`
    finishSfxPreview(key, track.id)
...
@media (max-width: 720px) { .audio-page { padding: 22px 16px; } .audio-grid { grid-template-columns: 1fr; } .track-card { align-items: stretch; flex-direction: column; } .track-controls { justify-content: space-between; } .mixer-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
</style>
