...
describe('browser scene roleplay runtime', () => {
  ...
  it('omits attacks from model context and authoritatively freezes story state', () => {
    ...
    expect(applied.session.current_node_id).toBe(secureDefinition.start_node_id)
    expect(applied.session.node_turns).toBe(0)
    expect(applied.session.total_turns).toBe(0)
    expect(applied.session.transcript[0].turn).toBe(1)
    expect(applied.response.outcome.transition).toBeNull()
  ...
  it('uses authored fallback signals for clean failures but never for attacks', () => {
    ...
    expect(attacked.session.node_turns).toBe(0)
    expect(attacked.session.total_turns).toBe(0)
