...
const projectDataDir = process.env.MONOGATARI_PROJECT_ROOT
  ? path.resolve(scriptDir, '..', process.env.MONOGATARI_PROJECT_ROOT)
  : path.join(rootDir, 'data')
const projectAssetsDir = path.join(projectDataDir, 'assets')
const distProjectAssetsDir = path.join(distDir, 'assets')
const projectEventsDir = path.join(projectDataDir, 'events')
const distProjectEventsDir = path.join(distDir, 'events')
const projectScenesDir = path.join(projectDataDir, 'scenes')
const distProjectScenesDir = path.join(distDir, 'scenes')
const projectDialoguesDir = path.join(projectDataDir, 'dialogue')
...
const projectKnowledgeDir = path.join(projectDataDir, 'knowledge')
...
const projectWebModelDir = path.join(projectDataDir, 'models', 'webgpu')
...
const webSecurityCsp =
  "default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' asset: http://asset.localhost data: blob:; media-src 'self' asset: http://asset.localhost data: blob:; font-src 'self' data:; connect-src 'self' asset: http://asset.localhost blob: https: http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:*; worker-src 'self' blob:; object-src 'none'; base-uri 'self'; form-action 'none'; frame-src 'none'; frame-ancestors 'none'"
