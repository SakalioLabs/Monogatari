...
test('checked-in repository JSON returns passing evidence', async () => {
  ...
  assert(evidence.jsonFileCount >= 288)
  ...
  assert.equal(projectJsonFiles.length, 632)
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'dialogue', 'chapter1_guided_vn.json')))
  ...
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'campaigns', 'volume5_campaign.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'campaigns', 'volume6_campaign.json')))
  ...
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_chapter1_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_chapter2_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_chapter3_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_chapter4_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_chapter5_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_finale_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume5_epilogue_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume6_chapter1_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume6_chapter2_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume6_chapter3_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume6_chapter4_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter1_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter2_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter3_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter4_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter5_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'roleplays', 'volume4_chapter6_roleplay.json')))
  ...
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume4_chapter2_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume4_chapter3_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume4_chapter4_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume4_chapter5_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume4_chapter6_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume5_chapter3_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume5_chapter4_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume5_chapter5_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume5_finale_roleplay.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'quality_suites', 'volume5_epilogue_roleplay.json')))
  assert(
    projectJsonFiles.includes(
      path.join(projectPrefix, 'quality_suites', 'volume6_chapter1_roleplay.json'),
    ),
  )
  assert(
    projectJsonFiles.includes(
      path.join(projectPrefix, 'quality_suites', 'volume6_chapter2_roleplay.json'),
    ),
  )
  assert(
    projectJsonFiles.includes(
      path.join(projectPrefix, 'quality_suites', 'volume6_chapter3_roleplay.json'),
    ),
  )
  assert(
    projectJsonFiles.includes(
      path.join(projectPrefix, 'quality_suites', 'volume6_chapter4_roleplay.json'),
    ),
  )
  assert(
    projectJsonFiles.includes(
  ...
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'characters', 'hans.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'characters', 'wolbach.json')))
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'characters', 'arcanletia_guild_clerk.json')))
  ...
  assert(projectJsonFiles.includes(path.join(projectPrefix, 'endings', 'volume3_witnessed_response_at_dungeon_gate.json')))
...
test('repository file walker is deterministic and excludes generated directories', async () => {
  ...
  assert(defaultRepositoryScanExcludedDirectories.includes('.tmp'))
  ...
  assert(!visited.includes(path.join(fixtureRoot, '.tmp')))
})
