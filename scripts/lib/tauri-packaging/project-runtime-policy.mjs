...
export async function collectTauriProjectRuntimeEvidence(options = {}) {
  ...
  const runtimeRootRequirements = [
    ...
    [tauriProjectWorkspaceSource, 'activate_project(&app, &state', 'validate projects before recording them as active'],
    ...
    [tauriEngineSource, 'let root_changed = state.set_project_data_root(path.clone()).await', 'rebind project managers after staged engine initialization'],
