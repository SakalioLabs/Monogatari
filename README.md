...
- Rust runtime saves use the backward-compatible `monogatari-game-save/v2` schema to restore scene history, dialogue cursor/local state, typed Rhai variables, character emotion/relationships/full memory, chat history, evaluations, safety traces, and triggered-event state. Quick-save and auto-save use bounded stable slots with staged overwrite recovery instead of accumulating random files, and save reads/writes enforce a 32 MiB limit.
