...
- Rhai script commands and direct `ScriptEngine` callers validate payload size and hidden control characters before execution or parsing, condition expressions run through a read-only Rhai engine, workflow validation catches invalid script state keys during authoring/import, shared script state keys are normalized to portable save-friendly names, and the shared script engine caps operations, recursion, expression depth, variables, functions, and module imports.
