...
- Rhai script commands and direct `ScriptEngine` callers validate payload size and hidden control characters before execution or parsing, condition expressions use shared 2,000-character/control-character validation and run through a read-only Rhai engine, workflow conditions can read relationship/evaluation context variables without mutating story state, Web/PWA workflow previews mirror local variable, flag, signed relationship, emotion, and weighted random-branch behavior for later branches, workflow validation catches invalid condition and script state-key config during authoring/import, shared script state keys are normalized to portable save-friendly names, and the shared script engine caps operations, recursion, expression depth, variables, functions, and module imports.
