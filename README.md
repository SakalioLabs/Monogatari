...
- Quality Suite reports show and export versioned audit evidence with run metadata, build commit ids, failed-scenario ids, category summaries, safety-signal counts, finalized guarded workflow output text, and workflow coverage summaries for release QA, customer review, and branch-coverage audits.
...
- Release artifact manifests can be generated with `node scripts/create-release-manifest.mjs` to capture Web/PWA and desktop installer artifact paths, SHA-256 checksums, checked-in release channel policy metadata, git source-state evidence, missing installer expectations, and verified installer signing evidence.
