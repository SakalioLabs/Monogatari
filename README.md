...
- **Agent Authoring Skill and MCP** - A repository Skill defines dependency-ordered visual-novel production, while the standard stdio MCP server exposes twelve schema-backed inspect/validate/read/Quality/transaction/package tools over the same headless authoring core. Writes are disabled by default; transactions require exact SHA preconditions plus a reviewed plan fingerprint, and package inspection/private runtime validation/output is confined to a separately fixed external directory while output additionally requires a reviewed content fingerprint. See [MCP_SERVER.md](docs/MCP_SERVER.md).
