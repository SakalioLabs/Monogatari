...
An AI-native engine for building real-time, scene-bound roleplay visual novels across Web/PWA and Windows packages.

## What It Is

Monogatari is an authoring workbench and runtime toolkit, not a game itself. Its primary story loop places the player in an authored scene node and accepts free-form input. A model generates each NPC response from the scene, character, goals, Knowledge, and bounded transcript; a separate evaluator proposes structured score/evidence changes; and a deterministic state machine alone chooses the next node or ending. Developers author this graph in `roleplays/`, while fixed `dialogue/` remains available for intentionally scripted sequences. Web packages run local models through WebGPU; desktop/server deployments use compatible local runtimes or generation-verified OpenAI-compatible services.

## Key Features

- **Real-Time Scene Roleplay** - Versioned scene nodes define character goals, player goals, Knowledge, score/evidence rules, inference budgets, deterministic transitions, timeouts, and endings while every NPC line is generated at runtime.
- **Low-Code Story Flow** - Drag-and-drop Workflows compose scripted dialogue, conditions, model generation, scoring, authored events, state changes, and scene transitions around the core roleplay loop.
...
- **Quality Suites** - Provider-free scenarios replay roleplay turns through the real state machine and assert endings, nodes, coverage, scores, and evidence; compatibility scenarios also cover character stability, prompt safety, Knowledge, Story Events, scoring, and Workflows. Replay proves deterministic rules, not live model quality.
...
- **Agent Authoring Skill and MCP** - A repository Skill defines dependency-ordered AI-roleplay production, while the standard stdio MCP server exposes fourteen schema-backed inspect/validate/read/Scene-Roleplay-preview/Workflow-preview/Quality/transaction/package tools over the same headless core. Writes are disabled by default and require exact SHA preconditions plus a reviewed plan fingerprint. See [MCP_SERVER.md](docs/MCP_SERVER.md).
...
- **Knowledge Base** - Keyword-indexed world lore, pinned character and related-entry references, deterministic context ordering, and one shared bounded authoring/Agent validation boundary that feeds consistent AI prompts.
...
- A prior RTX 3060 probe verified Qwen3.5 0.8B Text ONNX Q4 initialization and streamed Chinese generation through the packaged Transformers.js WebGPU runtime. The post-hardening main-stage Scene Roleplay path still requires a fresh physical GPU probe before its live generation is claimed.
...
- Dialogue loading validates strict fields, authoritative node-map IDs, all transition targets, reachability, character references, relationship deltas, terminal metadata, script bounds, and LLM prompt requirements through one shared headless authoring boundary before desktop activation or Agent transaction acceptance.
- Knowledge loading rejects unknown fields, non-canonical values, unsafe IDs, invalid importance/tags/metadata, duplicate IDs, missing/self relations, symlinked or oversized documents, and overlarge catalogs through one shared headless boundary. Desktop saves and hot reloads, engine initialization, package validation, and Agent transactions build the runtime index from that validated normalized catalog; legacy `relatedEntries` and creator-defined categories retain their authored meaning.
...
- Web/PWA builds package Scene Roleplay, scene, dialogue, ending, event, and renderer assets in `project-assets.json`, cache them for offline use, and expose the same roleplay main stage in development and production. Browser roleplay sessions validate/clamp evaluator output and compute transitions locally; scripted Dialogue remains covered by its existing browser/Rust state machines.
...
- The standard `monogatari-mcp` adapter passes real stdio child-process tests for handshake, fourteen schema-backed tools, scrubbed inspection, read-only core/delivery validation, source-bound provider-free Scene Roleplay and Workflow previews, bounded Quality execution, exact JSON reads, write refusal, reviewed-plan confirmation, one-writer exclusion, successful application, and rollback. Core acceptance loads strict roleplay/scene/character/Knowledge/ending references; package validation remains distinct from installation, rendered experience, and live generation.
...
- Playwright Chromium coverage independently exercises the desktop/mobile Scene Roleplay main stage and free-input layout without invoking a model, plus the existing authoring, 3D scene, scripted route, and ending probes; live WebGPU generation remains a separate hardware check.
...
+-- .agents/skills/        # Agent-discoverable AI roleplay authoring workflow
...
Agents can invoke `$author-visual-novel` from the repository Skill at `.agents/skills/author-visual-novel`. The Skill authors the canonical project graph in dependency order, requires Quality Suite evidence, and validates output through the same engine and release contracts used for human-authored projects. The checked-in [`Tideglass: Blue Frame`](docs/games/BLUE_FRAME_STORY_BIBLE.md) project is an end-to-end reference: its JSON graph was created and revised through MCP transactions, its user-supplied GLB environments entered through the fingerprinted binary importer, and its deterministic Workflow, Quality Suite, and desktop/mobile Playtest evidence exercise the same public contracts.

The shared `llm-authoring` crate also defines `monogatari-agent-project-transaction/v1`: a JSON-only multi-file plan/apply contract with create-or-exact-SHA preconditions, portable path containment, candidate validation, and rollback. Its package domain owns deterministic inventory, staged ZIP output, and bounded verification/extraction into an empty caller-owned root. Its headless core-runtime loader owns real character/dialogue managers plus validated normalized Knowledge runtime construction, shared Dialogue and Knowledge authoring constraints, scene/ending models, bounded catalog loading, inferred background-scene discovery, and cross-reference checks for both Tauri and Agent transports. `monogatari-mcp` exposes that core through thirteen standard stdio tools with a startup-fixed project root, read-only core/delivery validation, source-bound Workflow preview, shared Quality Suite execution, reviewed plan fingerprints, process-level project leases, document-level inspection, `core_runtime` candidate application, and reviewed package inspection/private validation/output inside a separately fixed external directory. Binary renderer assets remain outside JSON transactions and use `scripts/import-project-asset.mjs`, whose plan/apply fingerprint binds the source bytes, destination precondition, media validation, and atomic write.
...
Run `node scripts/verify-release.mjs` before describing generated content as a deliverable project. Current module coverage and remaining audit work are tracked in `docs/MODULE_VERIFICATION.md`.
