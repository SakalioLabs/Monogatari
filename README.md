...
- **Agent Authoring Skill and MCP** - A repository Skill defines dependency-ordered visual-novel production, while the standard stdio MCP server exposes eleven schema-backed inspect/validate/read/Quality/transaction/package tools over the same headless authoring core. Writes are disabled by default; transactions require exact SHA preconditions plus a reviewed plan fingerprint, and package inspection/output is confined to a separately fixed external directory while output additionally requires a reviewed content fingerprint. See [MCP_SERVER.md](docs/MCP_SERVER.md).
...
- The standard `monogatari-mcp` adapter passes real stdio child-process tests for handshake, eleven schema-backed tools, scrubbed inspection, read-only core and delivery validation, bounded Quality Suite execution with exact source fingerprints and structured pass/failure evidence, exact JSON reads, read-only refusal, reviewed-plan confirmation, one-writer exclusion, successful application, and rollback. Its package tools add a credential-free manifest preview, bounded fixed-root archive inspection, and fingerprint-confirmed ZIP output under one startup-fixed external directory, with traversal, damaged-archive, stale-review, and implicit-replacement rejection. Candidate application reports `core_runtime` only after real character/dialogue/knowledge managers load; strict scene/ending/Story Event/Workflow/Quality Suite catalogs load; inferred background scenes resolve; Workflow graphs and Quality expectations pass; and their project references pass. Delivery, archive inspection, and package generation remain distinct from runtime re-import and rendered visual acceptance.
