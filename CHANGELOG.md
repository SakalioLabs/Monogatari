...
- Moved the `.monogatari` manifest schema, inventory limits, deterministic package fingerprinting, file/directory topology checks, and portable cross-platform path rules from Tauri into `llm-authoring::project_package`. Seven pure protocol tests now run without Tauri, while desktop commands retain streaming ZIP I/O, atomic replacement, staged extraction, runtime reload, and non-overwriting import behavior.
