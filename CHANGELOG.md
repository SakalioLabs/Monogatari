...
- Scene Roleplay live turns are now transactional across both browser and desktop runtimes: model-generated NPC prose and the independent structured evaluation must both succeed before transcript, score, evidence, relationship, node, or ending state can change. NPC generation failure, evaluator failure, invalid evaluator JSON, and rejected model output leave the turn uncommitted and keep the player's input available for retry instead of substituting authored NPC dialogue or fallback scoring. ORT allocation failures are reduced-context retried by the WebGPU runtime, then surfaced as a bounded memory/action message without exposing `OrtRun` or `std::bad_alloc`.
