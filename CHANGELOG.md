...
- Added an in-story LLM NPC drawer to Playtest without making the deterministic mainline provider-dependent. Desktop sessions reuse the existing guarded Tauri chat path with character personality, pinned Knowledge, history, relationship updates, safety traces, and Story Event applications; Web/PWA sessions reuse the packaged Qwen WebGPU runtime through one shared bounded prompt/history domain that strips private-reasoning tags. Blue Frame now exposes 九号回声 from the active scene, pauses autoplay while the drawer is open, isolates composer keyboard input from story advancement, and carries desktop/mobile unit and Playwright evidence.
