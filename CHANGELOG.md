...
### Added
- Bound browser Character, Scene, Dialogue, and Ending authoring drafts to the active project's deterministic content scope, preventing stale sample drafts from replacing another project's Playtest catalogs. An unavailable authoring-preview target now starts the active project's primary Campaign or Scene Roleplay instead of leaving an empty or legacy Dialogue stage. Added the KonoSuba Volume 2 Chapter 1 production foundation with Eris, Dust, a winter-spirit field, two Knowledge boundaries, and reviewed renderer assets; the seven-node dynamic Roleplay remains explicitly pending rather than being represented as fixed Dialogue.
