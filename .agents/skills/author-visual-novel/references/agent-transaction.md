...
v1 accepts lowercase `.json` files beneath `assets`, `campaigns`, `characters`, `dialogue`, `endings`, `events`, `knowledge`, `locales`, `quality_suites`, `roleplays`, `scenes`, or `workflows`. Paths use bounded portable ASCII segments. Absolute paths, URI/drive prefixes, traversal, hidden segments, missing parent directories, symlinks, duplicate targets, and ASCII case collisions are rejected.
...
MCP read-only inspection reports `acceptance_level: "document"`. After staging, successful candidate application reports `acceptance_level: "core_runtime"` only if settings/documents pass; the real character/dialogue managers and shared normalized Knowledge builder load; strict scene, ending, Story Event, Scene Roleplay, Campaign, Workflow, and Quality Suite catalogs load; and every supported cross-catalog reference resolves. Otherwise every operation rolls back. Knowledge candidates additionally require known fields, canonical values, bounded content/metadata/catalogs, and complete non-self `related_entries`. Package, delivery-asset, live-model, and rendered-experience validation remain separate gates.
