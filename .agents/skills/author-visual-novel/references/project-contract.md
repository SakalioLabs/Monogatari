...
- Treat every roleplay node participant as a live NPC. Keep the primary and supporting IDs unique, select at most one present `speaker_id` per turn, load that speaker's own profile/Knowledge/relationship, retain the speaker in transcript evidence, and apply relationship deltas only to that speaker. A supporting NPC must not inherit the primary NPC's private goal, and no NPC may choose a route or ending.
